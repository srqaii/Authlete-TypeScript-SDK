...
export class Gm extends APIResource {
   ...
   * ```ts
   * const response =
   *   await client.gm.processGrantManagementRequest(
   *     'serviceId',
   *     {
   *       accessToken:
   ...
   * ```
   */
  processGrantManagementRequest(
    serviceID: string,
    body: GmProcessGrantManagementRequestParams,
    options?: RequestOptions,
  ): APIPromise<GmProcessGrantManagementRequestResponse> {
    return this._client.post(path`/api/${serviceID}/gm`, { body, ...options });
  }
}

export interface GmProcessGrantManagementRequestResponse {
  /**
...
export interface GmProcessGrantManagementRequestParams {
  /**
