...
export class Registration extends APIResource {
  ...
  retrieve(
    serviceID: string,
    body: RegistrationRetrieveParams,
    options?: RequestOptions,
  ): APIPromise<RegistrationRetrieveResponse> {
    return this._client.post(path`/api/${serviceID}/client/registration/get`, { body, ...options });
  }
...
export interface RegistrationRetrieveResponse {
  /**
   * The next action that the authorization server implementation should take.
   */
  action?: 'INTERNAL_SERVER_ERROR' | 'BAD_REQUEST' | 'CREATED';
